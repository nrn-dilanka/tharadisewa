...
const AdminDashboardScreen = ({ navigation }) => {
  ...
  const loadProducts = async () => {
    try {
      ...
      if (result.success) {
        const productsData = result.data.results || result.data || [];
  ...
  const handleSaveItem = async () => {
    ...
    try {
      ...
      if (modalType === 'product') {
        if (editingItem) {
          result = await productService.updateProduct(editingItem.id, formData);
        } else {
        ...
        if (editingItem) {
          result = await shopService.updateShop(editingItem.id, formData);
        } else {
        ...
        if (editingItem) {
          result = await customerService.updateCustomer(editingItem.id, formData);
        } else {
  ...
  const ItemCard = ({ item, type, onEdit, onDelete }) => {
    ...
    switch (type) {
      ...
      case 'product':
        title = item.name;
        subtitle = item.shop?.name || 'No Shop';
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <ItemCard
                item={item}
                type="product"
                onEdit={(product) => openEditModal('product', product)}
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <ItemCard
                item={item}
                type="shop"
                onEdit={(shop) => openEditModal('shop', shop)}
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <ItemCard
                item={item}
                type="customer"
                onEdit={(customer) => openEditModal('customer', customer)}
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            if (activeTab === 'users') {
              ...
              openCreateModal(activeTab.slice(0, -1)); // Remove 's' from end
            }
          }}
        >
      ...
      >
        <SafeAreaView style={styles.modalContainer}>
          ...
          <ScrollView style={styles.modalContent}>
            {/* Add form fields based on modal type */}
            <Text style={styles.comingSoon}>
              {modalType.charAt(0).toUpperCase() + modalType.slice(1)} form coming soon!
            </Text>
            <Text style={styles.comingSoonDesc}>
              This feature will allow you to create and edit {modalType}s directly from the app.
            </Text>
          </ScrollView>
